...
from protocols.wow.shared.utils.OpcodeLoader import load_auth_opcodes, load_world_opcodes
...
class ControlServer:
    ...
    def _build_completions(self) -> Tuple[List[str], List[str], List[str], List[str], List[str]]:
        ...
        try:
            ...
            version = cfg["version"]
            # From captures/debug
            for p in (get_captures_root() / "debug").glob("*.json"):
                opcodes_capture.add(p.stem)
                opcodes_all.add(p.stem)
            # From focus captures
            for p in (get_captures_root(focus=True) / "debug").glob("*.json"):
                opcodes_focus.add(p.stem)
                opcodes_all.add(p.stem)
            # From promoted def files
            def_dir = Path("protocols") / program / expansion / version / "data" / "def"
