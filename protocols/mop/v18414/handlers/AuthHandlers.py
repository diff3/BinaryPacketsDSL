...
from modules.crypto.SRP6Session import SRP6Session
...
def handle_AUTH_LOGON_CHALLENGE_C(client_socket, opcode, data: bytes):
    ...
    N = int(cfg["crypto"]["N"], 16)
    g = int(cfg["crypto"]["g"])

    session = SRP6Session(username, salt, verifier)
    fd = client_socket.fileno()
    srp6_sessions[fd] = session

    try:
        _, B_bytes = session.generate_B()
    except Exception as exc:
...
def handle_AUTH_LOGON_PROOF_C(client_socket, opcode, data: bytes):
    ...
    ok, M2, fields, k_bytes = session.verify_proof(A, M1)
    if not ok:
