...
class SRP6Core:
    ...
    # Generator value (constant in WoW SRP6)
    G: int = 7
    ...
    def server_verify(
        ...
        print("[SRP-DBG] u =", u_value.to_bytes(20, "little").hex())

        # --- compute S -------------------------------------------------
        s_bytes = self.server_compute_S(a_public, verifier, b_value, u_value)
        print("[SRP-DBG] S_server =", s_bytes.hex())

        # --- compute K -------------------------------------------------
        k_bytes = self.server_compute_K(s_bytes)
        print("[SRP-DBG] K_server =", k_bytes.hex())

        # --- compute M1 ------------------------------------------------
        m1_server = self.compute_M1(username_u, salt, a_public, b_public, k_bytes)
        print("[SRP-DBG] M1_server =", m1_server.hex())
        print("[SRP-DBG] M1_client =", m1_client.hex())
        ...
        print("[SRP-DBG] M2_server =", m2.hex())

        return True, m2

    # ======================================================================
    # MariaDB INSERT helper
    # ======================================================================

    def make_mariadb_insert(
        self,
        username: str,
        salt: bytes,
        verifier: bytes,
        ...
        """
        Generate a SQL INSERT statement for the SkyFire 'account' table.

        Returns:
            str: Fully formatted MariaDB INSERT statement.
        """
        _config = None
        ...
        return (
            ...
            f"    ('{username_u}', X'{salt_hex}', X'{verifier_hex}');"
        )